# Cowork / Claude Subagent Operating Model
...
Every agentic request enters through the router first. Triggers: `/subagent` or `/agent`, naming the router (`@router`, "route this ..."), a natural-language ask for an agentic or multi-step approach, and **any request that names a specific capability** (for example "as marketing ...") - the router triages it before handing off. In Claude Code / Cowork the router can also run as a subagent via the Agent/Task tool for multi-capability chains.
...
- Security to implementation: required fixes, accepted risks, and open exposure
  are kept separate.
- Desk research to positioning: when a marketing request depends on current
  customer language, audience phrasing, Reddit/forum/review language, persona
  feedback, or market-fit diagnosis, `desk-research` gathers the external voice
  of customer evidence before `positioning-messaging` translates it into
  claims, message hierarchy, and copy.
- Search to positioning: query language can inform visible copy, but voice and
  persuasion stay with positioning.
- Search metadata guardrail: when SEO/AEO tools report too few or too many
  keywords, route to `search-visibility` to fix metadata focus, length,
  crawlability, and machine-facing summaries. Do not create keyword lists or
  visible SEO scaffolding; use practical ranges for titles, descriptions,
  structured descriptions, and `llms.txt` while preserving approved page voice.
...
## 9. Cowork / Claude invocation specifics
...
- **Triggers:** `/subagent`, `/agent`, `@router`, "route this ...", or any natural-language ask for an agentic / multi-step approach.
